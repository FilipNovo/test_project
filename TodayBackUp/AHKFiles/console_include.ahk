...
class BensConsole
{
	...
	__New(logloc, name = "", color = "000000", fontColor = "15bb10", textBackgroundCol = "000000", opacity = 180, fontsize = 10, titlebar = true, topToBottom = false, prependDate = true, animate = false, clearOnClose = false, alwaysFlushToDisk = false, enableCommands = true, consoleMode = false, logCommands = true, nameInLog = false)
	{
		this.logloc := logloc
		if(isObject(name))
		{
	...
	log(msg)
	{
		...
		else
		{
			if(this.conshide)
			{
				...
				gui, % this.myGUIName ": show", % "x" this.posX " y" this.posY "w500 h200"
				this.conshide := false
			}
			this.applyContents()
		}
	...
	{
		; +E0x20 ; <-- click through!
		GUI, % this.myGUIName ": New", +Lastfound +ToolWindow +Resize, % this.myTitle ; 
		...
		{	
			...
			global titbar
			Gui, % this.myGUIName ": Add", Text, % "x" this.padding " y" this.padding " w" tmw  " vtitbar", % this.myTitle
		...
		Gui, % this.myGUIName ": Add", Edit, % "x" this.padding " y" this.startpos " w" this.tWmax " +Multi c" this.fontcol " HwndtLogContsID h200" vLogContent, % this.LogContents
		this.LogContsID := tLogContsID
		gui, % this.myGUIName ": show", % "x" this.posX " y" this.posY "w500 h230"
	...
	applyContents()
	{
		...
		Gui, % this.myGUIName ": Font", % "S" this.fontsize, Consolas
		Gui, % this.myGUIName ": Add", Edit, % "x" this.padding " y" this.startpos " w" this.tWmax " +Hidden h200 -E0x200 HwndtGhost", % this.LogContents
		this.Ghost := tGhost
