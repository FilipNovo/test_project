...
GetRange(ByRef x="",ByRef y="",ByRef w="",ByRef h="", manual=false)
{
 ...
 ; CoordMode, Mouse, %cmm%
		...
		Sleep, 100
		MouseGetPos, x, y
		ToolTip, Please hold down LButton key to select Mid point
	  }
	  Until GetkeyState("LButton","P")
      ;~ xStart := x - (w/2)
      ;~ yStart := x - (h/2)
	  ;~ Gui, Range:Show, NA x%xStart% y%yStart% w%w% h%h%
      ToolTip
      ;~ Gui, Range:Destroy
	  Gui, Canvas:Destroy
	  ; Clean the memory image and restore the initial state
